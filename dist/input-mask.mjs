/*!
 * input-mask v1.0.0
 * https://github.com/weslleycabral/input-mask#readme
...
var MaskElement = class {
  ...
  select(start2, end) {
    if (start2 == null || end == null || start2 === this.selectionStart && end === this.selectionEnd) return;
    try {
...
var ChunksTailDetails = class _ChunksTailDetails {
  ...
  appendTo(masked) {
    ...
    for (let ci = 0; ci < this.chunks.length; ++ci) {
      ...
      const stop2 = chunk.stop;
      let chunkBlock;
      if (stop2 != null && // if block not found or stop is behind lastBlock
      (!lastBlockIter || lastBlockIter.index <= stop2)) {
        if (chunk instanceof _ChunksTailDetails || // for continuous block also check if stop is exist
        masked._stops.indexOf(stop2) >= 0) {
          details.aggregate(masked._appendPlaceholder(stop2));
        }
        chunkBlock = chunk instanceof _ChunksTailDetails && masked._blocks[stop2];
      }
      if (chunkBlock) {
...
var MaskedPattern = class _MaskedPattern extends Masked {
  ...
  _findStopBefore(blockIndex) {
    ...
    for (let si = 0; si < this._stops.length; ++si) {
      const stop2 = this._stops[si];
      if (stop2 <= blockIndex) stopBefore = stop2;
      else break;
    }
    return stopBefore;
  }
...
// src/index.js
var MASK_SELECTOR = "[data-mask]";
var instances = /* @__PURE__ */ new WeakMap();
var rawBindings = /* @__PURE__ */ new WeakMap();
var mutedAttributeElements = /* @__PURE__ */ new WeakSet();
...
function syncRawHiddenValue(input, mask) {
  ...
  if (!binding) return;
  binding.hidden.value = (_a = mask.unmaskedValue) != null ? _a : "";
}
function setupRawSubmission(input, mask) {
  ...
  hidden.value = (_a = mask.unmaskedValue) != null ? _a : "";
...
function getCommonOptions(el) {
  ...
  const needsTransform = hasAttr(el, "data-mask-uppercase") || hasAttr(el, "data-mask-lowercase") || hasAttr(el, "data-mask-trim");
  if (needsTransform) {
...
function createObserver() {
  const pendingInit = /* @__PURE__ */ new Set();
  const pendingDestroy = /* @__PURE__ */ new Set();
  ...
  const queueTree = (node, action) => {
    ...
    if ((_a = node.matches) == null ? void 0 : _a.call(node, MASK_SELECTOR)) {
      action(node);
    }
...
if (typeof window !== "undefined") {
  window.InputMask = InputMask2;
}
var index_default = InputMask2;
export {
